...
namespace MyHome.DataRepositories.Tests
{
    ...
    public class PaymentMethodRepositoryTests
    {
        ...
        {
            var mock = RepositoryMocks.GetMockPaymentMethodRepository(new List<PaymentMethod>() { baseTestData });
        ...
        public void PaymentMethodRepository_GetAll_Returns_All_Data()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                expected.Add(new PaymentMethod(baseTestData.Id + i, $"{baseTestData.Name}::{i}"));
            }

...
namespace MyHome.DataRepositories.Tests
{
    ...
    public class IncomeCategoryRepositoryTests
    {
        ...
        {
            var mock = RepositoryMocks.GetMockIncomeCategoryRepository(new List<IncomeCategory> { _baseTestData });
        ...
        public void IncomeCategoryRepository_GetAll_Returns_All_Data()
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                expected.Add(new IncomeCategory(_baseTestData.Id + i, $"{_baseTestData.Name}::{i}"));
            }
